...
class VIP_APP:
    def __init__(self, width=500, height=300):
        ...
        ComboBox.add_command(label='搜狐视频', command=lambda: webbrowser.open('http://tv.sohu.com/'))
        ComboBox.add_command(label='芒果TV', command=lambda: webbrowser.open('http://www.mgtv.com/'))
        ComboBox.add_command(label='外挂系列-VIP视频破解', command=lambda: webbrowser.open('http://www.iqiyi.com/'))
        ComboBox.add_command(label='PPTV', command=lambda: webbrowser.open('http://www.bilibili.com/'))
        ComboBox.add_command(label='优酷', command=lambda: webbrowser.open('http://www.youku.com/'))
        ComboBox.add_command(label='乐视', command=lambda: webbrowser.open('http://www.le.com/'))
        ComboBox.add_command(label='土豆', command=lambda: webbrowser.open('http://www.tudou.com/'))
        ComboBox.add_command(label='A站', command=lambda: webbrowser.open('http://www.acfun.tv/'))
        ComboBox.add_command(label='B站', command=lambda: webbrowser.open('http://www.bilibili.com/'))

        # 设置控件
        # frame1
        lab = tk.Label(frame_1, text='请选择一个视频播放通道:', padx=10, pady=10)
        channel1 = tk.Radiobutton(frame_1, text='通道一', variable=self.n, value=1, width=10, height=3)
        channel2 = tk.Radiobutton(frame_1, text='通道二', variable=self.n, value=2, width=10, height=3)
